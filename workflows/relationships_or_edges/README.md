...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | Source Field / FHIR Mapping |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------|
| `HAS_MPI_RECORD`                  | Patient → MasterPatientIndex                   | Links local patient record to the enterprise golden MPI record for identity resolution                                                            | `Patient.link` → `Patient` |
| `IS_LINKED_TO`                    | Patient A → Patient B                          | Probabilistic soft link — candidate duplicate awaiting review                                                                                    | `Patient.link.type = "seealso"` |
...
| `IS_SPLIT_FROM`                   | NewPatient → OriginalPatient                   | Reverse of erroneous merge — identity split performed                                                                                            | Audit trail |
| `HAS_POTENTIAL_DUPLICATE`         | Patient A → Patient B                          | Temporary flag during automated MPI matching workflow                                                                                            | Internal MPI engine |
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | FHIR Mapping |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
...
| `AUTHORED_BY`                     | ClinicalNote → User (Doctor/NP/PA)             | Primary author of the note                                                                                                                        | `Composition.author` |
| `COSIGNED_BY`                     | ClinicalNote → User                            | Attending co-signature (e.g., resident note)                                                                                                      | `Composition.attester` |
| `HAS_SECTION`                     | ClinicalNote → SectionVertex                   | Structured sections: Chief Complaint, HPI, ROS, PE, Assessment/Plan                                                                               | `Composition.section` |
| `HAS_SUBSECTION`                  | Section → Subsection                           | Nested structure within a section (e.g., A/P → Differential Diagnosis)                                                                            | `section.section` |
| `MENTIONS_CONCEPT`                | ClinicalNote → SNOMED_Concept                  | NLP-extracted clinical entities (e.g., "cellulitis", "diabetes")                                                                                  | SNOMED CT |
| `HAS_PROBLEM_REFERENCE`           | ClinicalNote → Problem                         | Note references an active problem list item                                                                                                       | `Composition.section.entry` |
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | FHIR Mapping |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| `HAS_JOURNEY`                     | Patient → PatientJourney                       | Master timeline of all care events across lifetime                                                                                                | `CarePlan` + `EpisodeOfCare` |
...
| `HAS_ORDER`                       | PatientJourney → Order                         | All orders (labs, meds, imaging) across lifetime                                                                                                  | `ServiceRequest` |
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | LOINC / Example |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|
...
| `HAS_MICROBIOLOGY_CULTURE`        | LabResult → MicrobiologyCulture                | Culture & sensitivity results                                                                                                                     | `Observation` |
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | DICOM / FHIR |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
...
| Edge Label                        | Direction (From → To)                          | Description |
|-----------------------------------|------------------------------------------------|-----------|
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | Use Case |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|----------|
...
| Edge Label                        | Direction (From → To)                          | Description & Clinical Purpose                                                                                                                    | FHIR |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|------|
...
| Edge Label                        | Direction (From → To)                          | Example |
|-----------------------------------|------------------------------------------------|---------|
...
| Edge Label                        | Direction (From → To)                          | Description & Purpose                                                                                                                             | Source |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|--------|
...
| Edge Label                        | Direction (From → To)                          | Description & Example                                                                                                                             | SNOMED |
|-----------------------------------|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|--------|
...
| Edge Label                        | Direction (From → To)                          | FHIR Resource                                                                                 | HL7 v2 |
|-----------------------------------|------------------------------------------------|-----------------------------------------------------------------------------------------------|--------|
...
### Summary Legend – All Relationship Types Used in GraphDB Medical Model

| Relationship Type | Prefix / Pattern          | Meaning & Clinical Use Case                                                                 | Example |
|-------------------|---------------------------|---------------------------------------------------------------------------------------------|---------|
| **Ownership / Containment** | `HAS_*`                   | Entity directly owns or contains another entity (most common pattern)                      | `HAS_ENCOUNTER`, `HAS_LAB_RESULT`, `HAS_CLINICAL_NOTE` |
| **Identity & MPI**        | `HAS_MPI_*`, `IS_LINKED_TO`, `IS_MERGED_INTO` | Master Patient Index, deduplication, identity resolution, survivorship rules               | `HAS_MPI_RECORD`, `IS_MERGED_INTO`, `HAS_MATCH_SCORE` |
| **Authorship & Responsibility** | `AUTHORED_BY`, `PERFORMED_BY`, `RECORDED_BY` | Who created, signed, or performed an action                                                 | `AUTHORED_BY`, `PERFORMED_BY`, `COSIGNED_BY` |
| **Hierarchy / Ontology**  | `IS_A`                    | Taxonomic inheritance (SNOMED CT core relationship)                                        | `Cellulitis IS_A BacterialInfection` |
| **Composition**          | `PART_OF`                 | Whole-part anatomical or organizational structure                                           | `Left Ventricle PART_OF Heart` |
| **Anatomical / Clinical Attributes** | `HAS_FINDING_SITE`, `HAS_CAUSATIVE_AGENT`, `HAS_METHOD`, `HAS_SPECIMEN`, `HAS_DIRECT_SUBSTANCE` | SNOMED CT defining attributes for findings, disorders, procedures, substances              | `Pneumonia HAS_FINDING_SITE Lung` |
| **Qualifiers**            | `HAS_SEVERITY`, `HAS_LATERALITY`, `HAS_INTENT` | Additional descriptors for findings or procedures                                          | `Pain HAS_SEVERITY Severe` |
| **Care Continuum**        | `HAS_JOURNEY`, `TRANSITIONED_TO`, `READMITTED_WITHIN_30_DAYS` | Full patient journey, care transitions, readmission tracking                               | `Patient HAS_JOURNEY PatientJourney` |
| **Orders & Results**      | `HAS_ORDER`, `HAS_LAB_ORDER`, `HAS_IMAGING_ORDER`, `HAS_LAB_RESULT` | Clinical ordering workflow and result linkage                                               | `Encounter HAS_LAB_ORDER LabOrder` |
| **Imaging Specific**      | `HAS_SERIES`, `HAS_IMAGE`, `HAS_MODALITY`, `HAS_PROTOCOL`, `IS_CT_SCAN`, `IS_MRI` | Full radiology study structure and modality hierarchy                                      | `ImagingStudy HAS_SERIES ImagingSeries` |
| **Laboratory Specific**   | `HAS_COMPONENT`, `HAS_SPECIMEN`, `HAS_MICROBIOLOGY_CULTURE`, `HAS_ORGANISM` | Panel breakdown, specimen tracking, microbiology results                                   | `LabPanel HAS_COMPONENT LabResult` |
| **Acute & Emergency**     | `HAS_TRAUMA_ALERT`, `HAS_STROKE_ALERT`, `HAS_SEPSIS_ALERT`, `HAS_CODE_BLUE` | Time-critical activations and code events                                                  | `Encounter HAS_STROKE_ALERT StrokeAlert` |
| **Drug Safety**           | `INTERACTS_WITH`, `CAUSES_ADVERSE_REACTION`, `HAS_CONTRAINDICATION`, `REQUIRES_MONITORING` | Drug-drug interactions, allergies, contraindications, monitoring rules                    | `Warfarin INTERACTS_WITH VitaminK` |
| **Procedures**            | `HAS_PROCEDURE`, `PERFORMED_BY`, `ASSISTED_BY`, `IS_INTUBATION`, `IS_CENTRAL_LINE` | Surgical and bedside procedures with performer roles                                        | `Encounter HAS_PROCEDURE Intubation` |
| **Workflow & Governance** | `REQUIRES_CONSENT`, `REQUIRES_APPROVAL_BY`, `MUST_NOTIFY`, `HAS_ESCALATION` | Mandatory steps, approvals, notifications, escalation chains                               | `ControlledSubstance REQUIRES_APPROVAL_BY Pharmacist` |
| **Documentation**         | `HAS_SECTION`, `HAS_ADDENDUM`, `SUPERSEDES`, `MENTIONS_CONCEPT` | Structured clinical notes, amendments, NLP-extracted concepts                              | `ClinicalNote HAS_SECTION AssessmentPlan` |
| **Interoperability**      | `HAS_FHIR_*`, `SENT_AS_HL7`, `RECEIVED_AS_HL7` | Direct mapping to FHIR resources and HL7 v2 messages                                       | `Patient HAS_FHIR_PATIENT FHIR_Patient` |

**Color Coding in Practice (Recommended for Diagrams)**

| Color       | Relationship Category              |
|-------------|------------------------------------|
| Green       | Ownership / Containment (`HAS_*`) |
| Blue        | Identity & MPI                     |
| Purple      | SNOMED CT / Ontology               |
| Orange      | Acute & Emergency                  |
| Red         | Drug Safety & Interactions         |
| Yellow      | Workflow & Governance              |
| Teal        | Laboratory & Pathology             |
| Indigo      | Imaging & Radiology                |
| Gray        | Interoperability (FHIR/HL7)        |

This legend enables consistent modeling, powerful graph queries, and seamless integration with standards like **SNOMED CT**, **FHIR**, **HL7 v2**, and **LOINC**.

**Last Updated**: December 08, 2025  
**License**: MIT

This is the **definitive, unified, and complete** relationship model for a modern healthcare graph database.