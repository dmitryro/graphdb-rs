...
/// Represents a specific imaging (radiology) order placed for a patient as part of their care journey.
/// This edge links a PatientJourney (timeline) vertex to an ImagingOrder vertex.
/// Clinical Purpose: Radiology orders.
/// FHIR Mapping: ServiceRequest.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct HasImagingOrder {
    ...
    pub source_id: Uuid,
    /// ID of the target vertex (ImagingOrder)
    pub target_id: Uuid,

    /// The date and time the imaging order was placed or recorded.
    pub order_date: DateTime<Utc>,

    /// The current status of the order (e.g., 'active', 'completed', 'cancelled', 'draft').
    pub status: String,

    /// The CPT or local code for the primary imaging procedure being ordered (e.g., 'CT Abdomen').
    pub procedure_code: String,

    /// A brief reason or indication for why the imaging study was requested.
...
impl HasImagingOrder {
    pub const RELATIONSHIP_TYPE: &'static str = "HAS_IMAGING_ORDER";

    /// Creates a new, minimal HasImagingOrder instance.
    pub fn new(
        id: String,
        source_id: Uuid,
        target_id: Uuid,
...
impl EdgeSchema for HasImagingOrder {
    ...
    fn property_constraints() -> Vec<PropertyConstraint> {
        vec![
            ...
            PropertyConstraint::new("order_date", true)
                .with_description("The date and time the imaging order was placed (RFC3339)."),
            PropertyConstraint::new("status", true)
                .with_description("The current status of the imaging order (String)."),
            PropertyConstraint::new("procedure_code", true)
                .with_description("The CPT/code for the ordered imaging procedure (String)."),
            PropertyConstraint::new("reason", false)
                .with_description("Brief reason for the imaging order (String)."),
        ]
    }
}
// --- END EdgeSchema ---
...
impl ToEdge for HasImagingOrder {
    /// Converts the HasImagingOrder struct into a generic Edge structure for the graph database.
...
impl FromEdge for HasImagingOrder {
    fn from_edge(edge: &Edge) -> Option<Self> {
        ...
        let known_keys: &[&str] = &["id", "order_date", "status", "procedure_code", "reason"];
        ...
        // 2. Map the extracted and parsed data to the HasImagingOrder struct
        Some(HasImagingOrder {
